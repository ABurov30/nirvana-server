...
musicRouter.get('/uniqTags', async (req, res) => {
  try {
    const results = await Station.findAll({
      attributes: ['tags', [Sequelize.fn('SUM', Sequelize.col('votes')), 'votesSum']],
      group: ['tags'],
    ...
    const tags = results
      ...
      .split(',')
      .filter(
        (str) => str.length > 2 && str.length < 15 && !str.includes('fm') && !str.includes('radio'),
      );
...
musicRouter.get('/uniqCountry', async (req, res) => {
  try {
    const results = await Station.findAll({
      attributes: ['country', [Sequelize.fn('SUM', Sequelize.col('votes')), 'votesSum']],
      group: ['country'],
      order: [[Sequelize.col('votesSum'), 'DESC']],
      limit: 10,
    });
...
musicRouter.post('/search', async (req, res) => {
  try {
    ...
    if (name) {
      ...
      const result = await Station.findAll({
        where: {
          country: Sequelize.where(Sequelize.fn('LOWER', Sequelize.col('country')), {
            [Op.iLike]: `%${country.trim().toLowerCase()}%`,
          }),
          tags: Sequelize.where(Sequelize.fn('LOWER', Sequelize.col('tags')), {
            [Op.iLike]: `%${tags.trim().toLowerCase()}%`,
          }),
        },
        order: [['votes', 'DESC']],
